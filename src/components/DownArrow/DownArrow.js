...
const DownArrow = () => {
    ...
    const {radians, color} = useSpring({
        to: async (next) => {
            while (1) await next({radians: 1 * Math.PI, color: "black"})

        },
        immediate: false,
        from: {radians: 0, color: "white"},
        config: {precision: .1, velocity: 0, mass: 10, tension: 75, friction: 75},
        reset: true,
    });
    ...
    return (
        <Button
            onClick={onPress}
            focusRipple
            className={classes.downArrow_container} style={{border: `3px solid ${colorScheme.primary.light}`}}
        >
